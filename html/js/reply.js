...
$(document).ready(function () {
    ...
    $("#additionalPersons").on('change', function () {
        ...
        $("#extraPersons").empty();
        $("#sleepover").empty()
        $("#sleepover").append("<option value = 0>-- Hvor mange trenger overnatting? --</option>")
        $("#sleepover").append("<option value = 'ingen'>Ingen</option>")



        if (persons > 0 ) {
            $("#extraPersons").append("<div class='form-group'> <div class='col-sm-4'> <h5>Fint! Da vil vi gjerne ha navnet på de andre også.</h5> </div> </div>")
            for (i = 0; i < persons; i++) {
                $("#extraPersons").append("<div class='form-group'> <div class='col-sm-4'> <input type='name' id = 'person" + (i+2) +"'class='form-control' placeholder='Person " + (i+2) + "'> </div> </div>")
